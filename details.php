...
include 'PDO.php';
session_start();
if (empty($_SESSION) || $_SESSION['isLoggedIn'] == false) {
    $_SESSION['isLoggedIn'] = false;
    $_SESSION['message'] = "You need to login first to buy a ticket";
}

if (isset($_GET['details'])) {
    $numVersion = $_GET['numVersion'];

    $query = "SELECT * FROM evenement INNER JOIN version ON evenement.idEvenement = version.idEvenement  WHERE numVersion = :numVersion";
    $DATA = $DB->prepare($query);
    $DATA->bindParam(':numVersion', $numVersion);
    $DATA->execute();

    $evenement = $DATA->fetch();
}
...
<body class="bg-light">
    ...
    <main>
        ...
        <section>
            ...
            <?php
            } elseif ($_SESSION['isLoggedIn'] == false && $_SESSION['message'] != "You need to login first to buy a ticket") {
            ?>
                <div class="container margin-top">
            ...
            ?>
                <div class="container margin-top">
                    <div class="alert alert-warning d-flex align-items-center justify-content-between alert-dismissible fade show px-5" role="alert">
                        <div class="d-flex align-items-center justify-content-center gap-3">
        ...
        <section>
            <div class="container my-5">
                <div class="row">
                    <div class="col-8 bg-white rounded-top px-3 py-3">
                        <img src="img/<?php echo $evenement['image'] ?>" class="img-fluid w-75 mx-auto d-block" alt="<?php echo $evenement['titre'] ?>">
                    </div>
                    <div class="col-3 glass rounded shadow-lg px-3 py-3 ms-1 mb-1">
                        <h1 class="text-center">Bill</h1>
                        <hr>
                        <form action="bill.php" method="post">
                            <label for="normal ticket">normal ticket: <?php echo $evenement['tarifnormal'] ?> DH</label>
                            <input type="number" min=0 value="0" name="normal" id="normal" class="form-control my-2">
                            <label for="reduced ticket">reduced ticket: <?php echo $evenement['tarifReduit'] ?> DH</label>
                            <input type="number" min=0 value="0" name="reduced" id="reduced" class="form-control my-2">
                            <div class="d-flex justify-content-center mt-3">
                                <?php
                                if (isset($_SESSION['ticketLeft'])) {
                                    echo $_SESSION['ticketLeft'];
                                }
                                ?>
                            </div>
                            <div class="d-flex justify-content-center mt-3">
                                <input type="text" hidden name="numVersion" value="<?php echo $numVersion ?>">
                                <button type="submit" name="bill" class="btn btn-primary px-4 py-2">BUY</button>
                            </div>
                        </form>
                        <?php
                        if (isset($_SESSION['ticketLeft'])) {
                            echo '<div class="alert alert-danger mt-3" role="alert"> there is only ' . $_SESSION['ticketLeft'] . ' left for this event </div>';
                            unset($_SESSION['ticketLeft']);
                        }
                        ?>
                    </div>
                </div>
                <div class="row">
                    <div class="col-11 bg-white rounded-bottom px-3 py-3">
                        <span><?php echo $evenement['categorie'] ?></span>
                        <h1><?php echo $evenement['titre'] ?></h1>
                        <p><?php echo $evenement['description'] ?></p>
                        <p>Date: <?php echo $evenement['dateEvenement'] ?></p>
